...
void wls_transport_enable(bool enable) {
    ...
    if (enable) {
        if (host_get_driver() != &wireless_driver) {
            ...
            // TODO: This doesn't seem to compile on newer QMK versions
            // It seems to be referencing a (now) non-existent global variable
            // Commenting it out doesn't seem to do anything
